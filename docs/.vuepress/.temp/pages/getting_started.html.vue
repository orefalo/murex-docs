<template><div><h1>Language Tour</h1>
<h2>Table of Contents</h2>
<div id="toc">
<ul>
<li><a href="#introduction">Introduction</a>
<ul>
<li><a href="#readevalprint-loop">Read–Eval–Print Loop</a></li>
...
<li><a href="#the-bang-prefix">The Bang Prefix</a></li>
</ul>
</li>
<li><a href="#rosetta-stone">Rosetta Stone</a></li>
<li><a href="#basic-syntax">Basic Syntax</a>
<ul>
<li><a href="#quoting-strings">Quoting Strings</a></li>
...
<li><a href="#piping-and-redirection">Piping and Redirection</a>
<ul>
<li><a href="#pipes">Pipes</a></li>
<li><a href="#redirection">Redirection</a></li>
<li><a href="#redirecting-to-files">Redirecting to files</a></li>
...
<li><a href="#sub-shells">Sub-Shells</a></li>
<li><a href="#filesystem-wildcards-globbing">Filesystem Wildcards (Globbing)</a></li>
...
<li><a href="#external-executables">External Executables</a></li>
</ul>
</li>
<li><a href="#control-structures">Control Structures</a>
<ul>
<li><a href="#using-if-statements">Using <code v-pre>if</code> Statements</a></li>
<li><a href="#using-switch-statements">Using <code v-pre>switch</code> Statements</a></li>
<li><a href="#using-foreach-loops">Using <code v-pre>foreach</code> Loops</a></li>
<li><a href="#using-formap-loops">Using <code v-pre>formap</code> Loops</a></li>
</ul>
</li>
<li><a href="#stopping-execution">Stopping Execution</a>
<ul>
<li><a href="#the-continue-statement">The <code v-pre>continue</code> Statement</a></li>
<li><a href="#the-break-statement">The <code v-pre>break</code> Statement</a></li>
<li><a href="#the-exit-statement">The <code v-pre>exit</code> Statement</a></li>
<li><a href="#signal-sigint">Signal: SIGINT</a></li>
<li><a href="#signal-sigquit">Signal: SIGQUIT</a></li>
<li><a href="#signal-sigtstp">Signal: SIGTSTP</a></li>
</ul>
</li>
</ul>
</div>
<h2 id="introduction" tabindex="-1"><a class="header-anchor" href="#introduction" aria-hidden="true">#</a> Introduction</h2>
<p>Murex is a typed shell. By this we mean it still passes byte streams along<br>
POSIX pipes (and thus will work with all your existing command line tools) but<br>
in addition will add annotations to describe the type of data that is being<br>
written and read. This allows Murex to expand upon your command line tools<br>
with some really interesting and advanced features not available in traditional<br>
shells.</p>
<blockquote>
<p>POSIX is a set of underlying standards that Linux, macOS and various other<br>
operating systems support. Most typed shells do not work well with existing<br>
...
<p>If you want to learn more about the interactive shell then there is a dedicated<br>
...
<p>Shells need to <RouterLink to="/blog/split_personalities.html">balance scripting with an efficient interactive terminal</RouterLink><br>
interface. One of the most common approaches to solving that conflict between<br>
readability and terseness is to make heavy use of barewords. Barewords are<br>
ostensibly just instructions that are not quoted. In our case, command names<br>
and command parameters.</p>
<p>Murex also makes heavy use of barewords and so that places requirements on<br>
...
<p>Due to the expectation of shell commands supporting bareword parameters,<br>
expressions have to be parsed differently to statements. Thus Murex first<br>
parses a command line to see if it is a valid expression, and if it is not, it<br>
...
^ a function         ^ a method
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>In practical terms, functions and methods are executed in exactly the same way<br>
however some builtins might behave differently depending on whether values are<br>
passed via STDIN or as parameters. Thus you will often find references to<br>
functions and methods, and sometimes for the same command, within these<br>
documents.</p>
<h3 id="the-bang-prefix" tabindex="-1"><a class="header-anchor" href="#the-bang-prefix" aria-hidden="true">#</a> The Bang Prefix</h3>
<p>Some Murex builtins support a bang prefix. This prefix alters the behavior of<br>
those builtins to perform the conceptual opposite of their primary role.</p>
<p>For example, you could grep a file with <code v-pre>regexp 'm/(dogs|cats)/'</code> but then you<br>
might want to exclude any matches by using <code v-pre>!regexp 'm/(dogs|cats)/'</code> instead.</p>
<p>The details for each supported bang prefix will be in the documents for their<br>
...
<p>If you already know Bash and looking for the equivalent syntax in Murex, then<br>
our <RouterLink to="/user-guide/rosetta-stone.html">Rosetta Stone</RouterLink> reference will help you to<br>
...
<blockquote>
<p>It is important to note that all strings in expressions are quoted whereas<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>(<code v-pre>/#</code> was chosen because it is similar to C-style comments however <code v-pre>/*</code> is a<br>
...
<p>If any variables are unset then reading from them will produce an error (under<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>You can also force Murex to read the global assignment of <code v-pre>$foo</code> (ignoring<br>
...
<p>Environmental Variables are like global variables except they are copied to any<br>
...
<p>Like with global variables, you can force Murex to read the environmental<br>
variable, bypassing and local or global variables of the same name, by also<br>
...
<p>In general, Murex will try to infer the data type of a variable or pipe. It<br>
can do this by checking the <code v-pre>Content-Type</code> HTTP header, the file name<br>
extension or just looking at how that data was constructed (when defined via<br>
...
<p>In traditional shells, variables are expanded in a way that results in spaces<br>
be parsed as different command parameters. This results in numerous problems<br>
where developers need to remember to enclose variables inside quotes.</p>
<p>Murex parses variables as tokens and expands them into the command line<br>
arguments intuitively. So, there are no more accidental bugs due to spaces in<br>
file names, or other such problems due to developers forgetting to quote<br>
...
<p>Due to variables not being expanded into arrays by default, Murex supports an<br>
...
<p>Murex supports multiple different pipe tokens. The main two being <code v-pre>|</code> and<br>
<code v-pre>-&gt;</code>.</p>
<ul>
<li>
...
<li>
<p><code v-pre>-&gt;</code> displays all of the supported methods (commands that support the output<br>
of the previous command). Think of it a little like object orientated<br>
...
<p>In Murex scripts you can use <code v-pre>|</code> and <code v-pre>-&gt;</code> interchangeably, so there's no need<br>
to remember which commands are methods and which are not. The difference only<br>
applies in the interactive shell where <code v-pre>-&gt;</code> can be used with tab-autocompletion<br>
to display a shortlist of supported functions that can manipulate the data from<br>
the previous command. It's purely a clue to the parser to generate different<br>
autocompletion suggestions to help with your discovery of different commandline<br>
tools.</p>
<h3 id="redirection" tabindex="-1"><a class="header-anchor" href="#redirection" aria-hidden="true">#</a> Redirection</h3>
<p>Redirection of stdout and stderr is very different in Murex. There is no<br>
support for the <code v-pre>2&gt;</code> or <code v-pre>&amp;1</code> tokens,  instead you name the pipe inside angle<br>
brackets, in the first parameter(s).</p>
<p><code v-pre>out</code> is that processes stdout (fd1), <code v-pre>err</code> is that processes stderr (fd2), and<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Likewise you can redirect either stdout, or stderr to <code v-pre>/dev/null</code> via <code v-pre>&lt;null&gt;</code><br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>You can also create your own pipes that are files, network connections, or any<br>
...
<p>Aside from annotating variables upon definition, you can also transform data<br>
along the pipeline.</p>
<h4 id="cast" tabindex="-1"><a class="header-anchor" href="#cast" aria-hidden="true">#</a> Cast</h4>
<p>Casting doesn't alter the data, it simply changes the meta-information about<br>
...
<ul>
<li>
<p>string sub-shells, <code v-pre>${ command }</code>, take the results from the sub-shell and<br>
return it as a single parameter. This saves the need to encapsulate the shell<br>
inside quotation marks.</p>
</li>
<li>
<p>array sub-shells, <code v-pre>@{ command }</code>, take the results from the sub-shell<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>The reason Murex breaks from the POSIX tradition of using backticks and<br>
parentheses is because Murex works on the principle that everything inside<br>
...
<p>While glob expansion is supported in the interactive shell, there isn't<br>
auto-expansion of globbing in shell scripts. This is to protect against<br>
...
<p>In <a href="https://en.wikipedia.org/wiki/Bash_(Unix_shell)#Brace_expansion" target="_blank" rel="noopener noreferrer">bash you can expand lists<ExternalLinkIcon/></a><br>
using the following syntax: <code v-pre>a{1..5}b</code>. In Murex, like with globbing, brace<br>
expansion is a function: <code v-pre>a: a[1..5]b</code> and supports a much wider range of lists<br>
...
<p>You can create &quot;aliases&quot; to common commands to save you a few keystrokes. For<br>
example:</p>
...
<p><code v-pre>private</code> functions are like <a href="#public-functions">public functions</a> except they<br>
...
<p>External executables (including any programs located in <code v-pre>$PATH</code>) are invoked<br>
via the <code v-pre>exec</code> builtin (<RouterLink to="/commands/exec.html">read more</RouterLink>) however if a command<br>
isn't an expression, alias, function nor builtin, then Murex assumes it is an<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>switch</code> supports a flexible variety of different usages to solve different<br>
...
<p><code v-pre>continue</code> will terminate execution of an inner block in iteration loops like<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>continue</code> requires a parameter to define while block to iterate on. This means<br>
...
<p><code v-pre>break</code> will terminate execution of a block (eg <code v-pre>function</code>, <code v-pre>private</code>, <code v-pre>if</code>,<br>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>break</code> requires a parameter to define while block to end. Thus <code v-pre>break</code> can be<br>
considered to exhibit the behavior of <em>return</em> as well as <em>break</em> in other<br>
languages:</p>
...
<p>Terminates Murex. <code v-pre>exit</code> is not scope aware; if it is included in a function<br>
...
<p>Sending SIGQUIT will terminate all running functions in the current Murex<br>
