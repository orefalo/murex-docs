...
<ul>
<li>mkarray (<code v-pre>a</code> et al) no longer returns an error if the start and end ranges<br>
...
<ul>
<li>
<p>Strict variables now supported (like <code v-pre>set -u</code> in Bash). This will mean any<br>
variables passed that haven't been initialized beforehand will cause that<br>
pipeline to fail. Currently this is opt in, a future release of Murex will<br>
flip that and make it opt out. So take this opportunity to enable it in your<br>
...
<li>
<p>New builtin: <code v-pre>datetime</code>. This builtin allows you to convert date and/or time<br>
strings of one format into strings of another format. <code v-pre>datetime</code> is a<br>
supercharged alternative to the UNIX command <code v-pre>date</code> aimed at making scripting<br>
easier.</p>
</li>
<li>
<p>mkarray (<code v-pre>a</code> et al) now supports dates. eg <code v-pre>[01-Jan-20..05-May-21]</code>. If no<br>
start nor end date appears then mkarray assumes range starts or ends at<br>
current date.</p>
</li>
<li>
<p><code v-pre>openagent</code> profile for <code v-pre>image</code> data types has been improved. Murex now<br>
better supports tmux and iTerm2.</p>
</li>
<li>
<p><code v-pre>runtime --config</code> now displays <code v-pre>FileRef</code> for every <code v-pre>set</code> as well as <code v-pre>define</code>,<br>
named <code v-pre>FileRefSet</code> and <code v-pre>FileRefDefine</code> respectively. So you can now easily<br>
...
<li>
<p>Prompt now defaults to only displaying current directory rather than the full<br>
...
<li>
<p>Parser updated to better support multiline pipelines where the newline is<br>
escaped and a comment exists after <a href="https://github.com/lmorg/murex/issues/379" target="_blank" rel="noopener noreferrer">issue #379<ExternalLinkIcon/></a>.<br>
This only applies to shell scripts, the interactive terminal hasn't yet been<br>
updated to reflect this change.</p>
</li>
<li>
<p>Fixed regression bugs with autocomplete parameters that affected some dynamic<br>
blocks.</p>
</li>
<li>
<p><code v-pre>readline</code> now caches syntax highlighting and hint text to improve the<br>
responsiveness of the interactive terminal. This is set to a hard limit of<br>
200 cached items however that will be a configurable metric in a future<br>
release. Also planned for the future is caching autocompletion suggestions.</p>
</li>
<li>
<p>Loading message added for the default profile, ie the one that is compiled<br>
into and thus shipped with Murex.</p>
</li>
<li>
<p>Fixed bug with <code v-pre>fid-list</code> and <code v-pre>jobs</code> where they were outputting the <code v-pre>p.Name</code><br>
struct rather than <code v-pre>p.Name.String()</code>. This lead to the process name appearing<br>
...
<ul>
<li>
<p>Thread safe copying of parameters upon fork. The previous code never actually<br>
generated any race conditions and I don't think ever could. However it was<br>
ambiguous. This new code makes the copy more explicit and appears to have<br>
also brought some minor performance improvements in benchmarks too.</p>
</li>
<li>
<p>Behavioural test framework has been refactored to make it easier to add new<br>
behavioural tests.</p>
</li>
<li>
...
<li><RouterLink to="/commands/a.html"><code v-pre>a</code> (mkarray)</RouterLink>:<br>
...
<li><RouterLink to="/commands/datetime.html"><code v-pre>datetime</code> </RouterLink>:<br>
A date and/or time conversion tool (like <code v-pre>printf</code> but for date and time values)</li>
<li><RouterLink to="/commands/fid-list.html"><code v-pre>fid-list</code></RouterLink>:<br>
Lists all running functions within the current Murex session</li>
<li><RouterLink to="/commands/ja.html"><code v-pre>ja</code> (mkarray)</RouterLink>:<br>
...
<li><RouterLink to="/commands/openagent.html"><code v-pre>openagent</code></RouterLink>:<br>
...
<li><RouterLink to="/commands/ta.html"><code v-pre>ta</code> (mkarray)</RouterLink>:<br>
